			...
			if ( urlNum != null ) {
				...
				switch ( pageLang ) {
					...
					case 'es':
						...
						fileExt = '.png';
						if (
							( langNum >= 217 && langNum <= 275 && langNum ==! 246 )
							||
							( langNum >= 311 && langNum <= 315 )
							||
							( langNum >= 351 && langNum <= 354 )
						) fileExt = '.jpg';
						break;
					case 'hu':
...
/* кнопки тулбара */
	...
	const helpButton = h ( 'button', {
		...
		h ( 'a', {
			href: `/${pageLang}/wiki/12balloons`,
			target: '_blank'
		}, [
			h ( 'img', {
				src: `${cotanPath}help.png`
			} ), LOCALIZED . HELP,
		] ),
	] );

	const cotanToolbar = h ( 'div', {
		className: 'cotan-toolbar'
	}, [
		cotanSaveButton,
		cotanCancelButton,
		helpButton
